...
if (isset($_REQUEST['ajax_request']) && $_REQUEST['ajax_request'] == true) {
    ...
    if (isset($_REQUEST['log_data'])) {
        ...
        if ($_REQUEST['type'] == 'slow') {
            $pdo = PMA_getJsonForLogDataTypeSlow($start, $end);
            PMA_Response::getInstance()->addJSON('message', $pdo);
            exit;
        }

        if ($_REQUEST['type'] == 'general') {
            $pdo = PMA_getJsonForLogDataTypeGeneral($start, $end);
            PMA_Response::getInstance()->addJSON('message', $pdo);
            exit;
        }
    }
    ...
    if (isset($_REQUEST['query_analyzer'])) {
        $pdo = PMA_getJsonForQueryAnalyzer();
        PMA_Response::getInstance()->addJSON('message', $pdo);
        exit;
    }
}
