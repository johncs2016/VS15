<?php
	...
	$limit = 20;
	$totrecs = tblexercise::getRecordCount();
	$pages = ceil($totrecs / $limit);
	$page = (isset($_GET['page']) ? preg_replace('#[^0-9]#i', '', $_GET['page']) : 1);
	$page = ($page < 1) ? 1 : (($page > $pages) ? $pages : $page);
	$start = ($page - 1) * $limit;
	$objects = ($totrecs < $limit) ? tblexercise::getAll() : tblexercise::getAll($start, $limit);
...
<html>
	<head>
		<meta charset="UTF-8">
		<title>Display Daily Activities</title>
		<?php
...
<?php
	echo "\t\t<div id=\"lhead\">\n";
	echo "\t\t\t<h1 id=\"ltitle\">List of Daily Activities For {$details->fullname()}</h1>\n";
		...
		<div id="data">
			<table>
				<tr class="dhead">
					...
					<th class="hodd">Activity</th>
					<th class="hedit"></th>
					<th class="hdelete"></th>
				</tr>
<?php
	foreach ($objects as $object) {
		$activity = tblactivities::get($object->getActivityID());
		...
		echo "\t\t\t\t\t<td class=\"dodd activity\"><strong>{$activity->getActivity()}</strong></td>\n";
		echo "\t\t\t\t\t<td class=\"dedit\"><a href=\"exerciseform.php?id={$object->getid()}\"><img class=\"dicon\" src=\"../images/edit.png\" alt=\"Edit\"> Edit</a></td>\n";
		echo "\t\t\t\t\t<td class=\"ddelete\"><a href=\"delete_exercise.php?id={$object->getid()}\" onclick=\"return confirm('Do You Really Want To Delete This Record?')\"><img class=\"dicon\" src=\"../images/delete.png\" alt=\"Edit\"> Delete</a></td>\n";
		echo "\t\t\t\t</tr>\n";
	}
?>
			<tr><td colspan="6" class="daddrec"><a href="exerciseform.php">Add New Record</a></td></tr>
			</table>
		</div>
		<div id="footer">
			<a href="weekly_exercise.php" class="button">Weekly Activity Summary</a>
