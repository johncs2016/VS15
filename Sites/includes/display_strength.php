<?php
	...
	$mindate = DateTime::CreateFromFormat('Y-m-d H:i:s', tblstrength::getMinMaxValue('obsdate', false, false, true) . ' 00:00:00');
	$maxdate = DateTime::CreateFromFormat('Y-m-d H:i:s', tblstrength::getMinMaxValue('obsdate', false, false, false) . ' 00:00:00');
	$props = array(
		"_id"			=>	0,
		"_equipname"	=>	"All Equipment"
	);
	$all_equipment = array();
	$all_equipment[] = new tblstrengthequipment($props);
	...
	$limit = 20;
	$totrecs = tblstrength::getRecordCount($where);
	$pages = ceil($totrecs / $limit);
	$page = (isset($_GET['page']) ? preg_replace('#[^0-9]#i', '', $_GET['page']) : 1);
	$page = ($page < 1) ? 1 : (($page > $pages) ? $pages : $page);
	$start = ($page - 1) * $limit;
	$objects = ($totrecs < $limit) ? tblstrength::getAll(0 , $totrecs, $where) : tblstrength::getAll($start, $limit, $where);
...
<html>
	<head>
		<meta charset="UTF-8">
		<title>Display Strength Exercises</title>
		<?php
...
<?php
	echo "\t\t<div id=\"lhead\">\n";
	echo "\t\t\t<h1 id=\"ltitle\">List of Strength Exercises For {$details->fullname()}</h1>\n";
...
<?php
	foreach ($objects as $object) {
		...
		echo "\t\t\t\t\t<td class=\"deven weight\"><strong>{$eweight}</strong></td>\n";
		echo "\t\t\t\t\t<td class=\"dodd sets\"><strong>{$sets}</strong></td>\n";
		echo "\t\t\t\t\t<td class=\"deven reps\"><strong>{$reps}</strong></td>\n";
		echo "\t\t\t\t\t<td class=\"dedit\"><a href=\"strengthform.php?id={$object->getid()}\"><img class=\"dicon\" src=\"../images/edit.png\" alt=\"Edit\"> Edit</a></td>\n";
		echo "\t\t\t\t\t<td class=\"ddelete\"><a href=\"delete_strength.php?id={$object->getid()}\" onclick=\"return confirm('Do You Really Want To Delete This Record?')\"><img class=\"dicon\" src=\"../images/delete.png\" alt=\"Edit\"> Delete</a></td>\n";
		echo "\t\t\t\t</tr>\n";
	}
?>
			<tr><td colspan="12" class="daddrec"><a href="strengthform.php">Add New Record</a></td></tr>
			</table>
		</div>
		<div id="footer">
			<a href="dashboard.php" class="button">Back To Dashboard</a>
			<a href="display_strength_progs.php" class="button">User Program</a>
