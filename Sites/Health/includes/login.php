<?php
	...
	if(isset($_POST['loginsubmit']) && $_POST['loginsubmit'] == "true") {
		...
		if(!$errors) {
			$sql = "SELECT TOP 1 id, username, password FROM tblusers WHERE username = ? AND password = ?";
			$db = tblusers::getConnection();
			$query = $db->prepare($sql);
			$query->execute(array($loginuser, md5($loginpassword)));
			$row = $query->fetch(PDO::FETCH_ASSOC);
			if(!empty($row)) {
				$user = tblusers::get($row['id']);
				$user->updateProperty("_sessionid", session_id());
				header("Location: " . $protocol . $server . "/Health");
				exit;
			} else {
