<?php
	...
	abstract class pdoClass {
		...
		public static function getVersion() {
			$db = self::getConnection();
			$result = $db->query("SHOW VARIABLES WHERE Variable_name = 'version'");
			$row = $result->fetch(PDO::FETCH_ASSOC);
			return (float)substr($row['Value'], 0, 3);
		}
...
//				die();
		...
		private static function getWhere($wh = null) {
			...
			try {
				$db = self::getConnection();
				$rows = $db->query(self::getSelect() . $where . $get_order . $limit, PDO::FETCH_ASSOC);
